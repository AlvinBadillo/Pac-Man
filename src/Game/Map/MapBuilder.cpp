...
MapBuilder::MapBuilder(string p){
...
//I think this is what i need to change to change map
    ...
    for (int i = 0; i < mapImage.getWidth(); i++) {
        for (int j = 0; j < mapImage.getHeight(); j++) {
            ...
            if(currentPixel == boundBoundBlock){
                ...
                Player* PacMan = new Player(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager, spritePlayer);
				...
				else if(dotsCount == 280 || dotsCount == 14 || dotsCount == 270){
...
void MapBuilder::setSpriteImg(string s){
	this->spritePlayer = s;
}
string MapBuilder::getSpriteImg(){
	return spritePlayer;
}